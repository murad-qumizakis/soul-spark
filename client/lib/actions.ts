...
export async function getProjectBySlug(slug: string): Promise<{ success: true; data: Project } | ErrorResponse> {
  try {
    const response = await fetch(API_URLS.projects.getBySlug(slug), {
      cache: "no-store",
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return handleApiError(new Error("Project not found"), "fetch project")
      }
    ...
    // Reuse projectSchema for validation
